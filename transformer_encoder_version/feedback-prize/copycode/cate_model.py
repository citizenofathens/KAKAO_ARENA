...
# 허깅페이스의 트랜스포머 패키지에는 BertConfig, BertModel 클래스 임포트
...
class CateClassifier(nn.Module):
    ...
    def __init__(self,cfg):
        ...
        # 분류기(Classifier 생성기
        # input : batch_size
        # Linear input feature :  batch_size x hidden_size , layer(weight) : (2*hidden_size(input_dimension) , hidden_size(output_dimension))
        # fine-tuning
        # 원래의 input vector는 comb_vec 으로 cfg.hidden_size * 2 이지만 img_feat를 제외하였으므로 cfg.hidden_size 만을 활용한다
        def get_cls(target_size=3):
...
#                nn.Linear(cfg.hidden_size*2 , cfg.hidden_size),
    ...
    def forward(self,  token_ids, token_mask, token_types ,   label=None):
        """
        매개변수
        token_ids: 전처리된 상품명을 인덱스로 변환하여 token_ids 를 만들었음
        token_amsk : 실제 token_ids 의 개수만큼은 1, 나머지는 0으로 채움
        ...
        text_output = self.text_encoder(token_ids , token_mask , token_type_ids=token_types)

        # 시퀀스 중 첫 타임스탭의 hidden state만 사용.
        text_vec  =text_output.last_hidden_state[:, 0]
        ...
        b_pred = self.b_cls(text_vec)
        # 결합된 벡터로 중카테고리 확률분포 예측
        # m_pred = self.m_cls(text_vec)
        # # 결합된 벡터로 소카테고리 확률분포 예측
        # s_pred = self.s_cls(text_vec)
        # # 결합된 벡터로 세카테고리 확률분포 예측
        # d_pred = self.d_cls(text_vec)

        # 데이터 페러럴 학습해서 GPU 메모리를 효율적으로 사용하기 위해
        # loss 를 모델 내에서 계산함
        if label is not None:
            # 손실(loss) 함수로 CrossEntropyLoss를 사용
            # label의 값이 -을 가지는 샘플은 loss계산에 사용 안함
            # target에 대해 one_hot encoding을 하고 nll_loss 적용 후 logsoftmax 값을 return
            # crossentroyloss에서 softmax를 하므로
            loss_func = nn.CrossEntropyLoss(ignore_index=-1)
            # label 은 batch_size x 4 를 (batch_size x1) 4개로 만ㄷ름
            b_label = label.split(1,1)
            # 대카테고리의 예측된 확률분포와 정답확률 분포의 차이를 손실로 반환
            # label 이 원핫인코딩 안된상태

            # 예측값과 손실값
            b_loss = loss_func(b_pred, b_label[0].view(-1))
            ...
            # s_loss = loss_func(m_pred, m_label.view(-1))
